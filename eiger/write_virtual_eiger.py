...
def get_frames_per_h5(master_file, verbose=False):
    ...
    with h5py.File(master_file, "r") as h:
...
def scoreFile_to_virtual(score_file, min_score, out_name, min_nspots=10):
    ...
    if len(entries)==0:
        print("Not entries in file!")
        return
    scores = entries[:,SCORE_POS].astype(int)
    nspots = entries[:,NSPOTS_POS].astype(int)
...
def main(fname_info, data_dir, out_name, zero_based_indexing=True):
    ...
    frame_per_master = {}
    for i_name, name in enumerate(unique_masters):
        frame_per_master[name] = get_frames_per_h5(name)
        print(i_name, len(unique_masters))
    ...
    for f in unique_masters:
        try:
            ...
            err_s = str(err)
            if "Too many open files" in err_s:
                keys = list(open_masters)
                # remove a few keys  so we can still open more hdf5s later...
                for k in keys[:10]:
                    open_masters.pop(k)
                break
            else:
    ...
    for i_img, (master_name, dset_idx) in enumerate(zip(all_master_fnames, all_dset_inds)):
        ...
        if master_name in open_masters:
            data_grp = open_masters[master_name]
        else:
            data_grp = h5py.File(master_name, 'r')[MASTER_ENTRY]
        ...
        if source_key not in sources:
            source_shape = data_grp[link_name].shape
